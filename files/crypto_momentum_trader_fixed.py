#!/usr/bin/env python3
"""
Crypto Momentum Trader - Fixed Version with Direct Order Execution
Bypasses the faulty KalshiClient wrapper and uses direct API calls
"""
...
# Logging
LOG_FILE = "data/crypto_momentum_trading_fixed.jsonl"
POSITIONS_FILE = "data/crypto_momentum_positions_fixed.json"
PERFORMANCE_FILE = "data/crypto_momentum_performance_fixed.json"
...
# ============================================================
# DIRECT API FUNCTIONS (BYPASS FAULTY CLIENT)
# ============================================================

def place_order_direct(auth, ticker: str, side: str, count: int, price: int) -> dict:
    """Place order using direct API call with correct endpoint and payload."""
    try:
...
def execute_momentum_trade(auth, opportunity, available_balance):
    """Execute a momentum trade using direct API call."""
    ...
    try:
        ...
        # Use direct API call instead of faulty client
        result = place_order_direct(auth, ticker, direction.lower(), contracts, entry_price)
        
        if "error" in result:
            log_event("execution_failed", {
                "ticker": ticker,
                "error": f"Order failed: {result['error']} - {result.get('detail', '')}"
            })
            print(f"   ❌ Order failed: {result['error']} - {result.get('detail', '')}")
            return result
...
def run_crypto_momentum_trader():
    """Run the crypto momentum trader with fixed execution."""
    print("=" * 80)
    print("🚀 CRYPTO MOMENTUM TRADER - FIXED VERSION")
    print("💰 Direct API Execution (Bypasses Faulty Client)")
    print("=" * 80)
    ...
    try:
        ...
        print(f"🔧 Using direct API calls (fixed execution)")
    ...
    while dt.now() < end_time:
        try:
            ...
            if not all_opportunities:
                ...
                if active_count < 3:  # Max 3 concurrent positions
                    ...
                    print(f"📊 Max concurrent positions reached (3)")
    ...
    log_event("momentum_session_complete", {
        "strategy": "crypto_momentum_fixed",
