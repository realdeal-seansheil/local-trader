#!/usr/bin/env python3
"""
Simple 15-Minute Crypto Trader - CORRECTED
Just finds and executes on the active 15-minute crypto markets
Uses the correct authentication method from the 48-hour bot
"""
...
# ============================================================
# AUTHENTICATION FUNCTIONS (CORRECTED FROM 48-HOUR BOT)
# ============================================================

def get_headers(auth, method: str, path: str) -> dict:
    """Generate authenticated headers using the correct method from 48-hour bot."""
    ...
    if auth.private_key:
        sig_bytes = auth.private_key.sign(
            msg.encode(),
            padding.PSS(  # CORRECT: Use PSS like the 48-hour bot
...
def place_order_direct_correct(auth, ticker: str, side: str, count: int, price: int) -> dict:
...
def execute_trade_correct(auth, market):
    """Execute a trade using the correct authentication method."""
    ...
    if best_trade:
        ...
        # Place the order using correct authentication
        result = place_order_direct_correct(auth, ticker, direction.lower(), POSITION_SIZE, price)
        ...
        if "success" in result:
            ...
            with open('data/simple_trades_correct.jsonl', 'a') as f:
...
def run_simple_15min_trader_correct():
    """Run the simple 15-minute crypto trader - CORRECTED."""
    print("=" * 70)
    print("🚨 SIMPLE 15-MINUTE CRYPTO TRADER - CORRECTED")
    print("💰 Finds and executes on active 15-minute crypto markets")
    print("⚡ Real-time market detection and trading")
    print("🔧 Fixed authentication using PSS padding")
    print("=" * 70)
    ...
    try:
        ...
        # Test authentication with correct method
        print(f"\n🧪 TESTING AUTHENTICATION...")
        
        # Test balance access with correct headers
        timestamp = str(int(dt.now().timestamp() * 1000))
        ...
        if active_markets:
            ...
            test_result = place_order_direct_correct(auth, test_ticker, 'yes', 1, test_price)
            
            if "success" in test_result:
                print(f"✅ TEST ORDER SUCCESSFUL!")
                print(f"   Order ID: {test_result.get('order_id', 'unknown')}")
            else:
                ...
                if active_markets:
                    ...
                    for market in active_markets:
                        ...
                        if best_trade and volume >= MIN_VOLUME:
                            ...
                            # Execute the trade
                            success = execute_trade_correct(auth, market)
...
if __name__ == "__main__":
    run_simple_15min_trader_correct()
